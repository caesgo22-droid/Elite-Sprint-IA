...
const VideoAnalyzer: React.FC = () => {
  ...
  const handleAutoCapture = async () => {
    ...
    try {
        ...
        // CRITICAL FIX: Monotonically increasing timestamps for MediaPipe
        for (let i = 0; i <= scanSteps; i++) {
            const time = (duration / scanSteps) * i;
            video.currentTime = time;
            
            // Wait for video frame seek
            ...
            // Process with monotonically increasing synthetic timestamp to avoid Graph errors
            const syntheticTimestamp = i * 100; 
            const result = poseLandmarker.detectForVideo(video, syntheticTimestamp);
        ...
        if (tempHistory.length === 0) {
            throw new Error("No se detectó ningún cuerpo en el video. Asegúrate de que el atleta sea visible de cuerpo completo.");
        }
        ...
        setStatusMessage(analysisMode === 'External' ? "Deep Intelligence..." : "Analizando Técnica...");
        ...
        alert(e.message || "Fallo en la captura biomecánica.");
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20 px-2">
       <div className="flex justify-between items-end border-b border-slate-800 pb-4">
          <div>
            <h2 className="text-2xl font-black text-white uppercase">Laboratorio Bio</h2>
       ...
       {!previewUrl ? (
           ...
           <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 p-1.5 bg-slate-900 rounded-3xl border border-slate-800 shadow-xl">
                    <button onClick={() => setAnalysisMode('Personal')} className={`py-4 rounded-2xl flex flex-col items-center gap-1 transition-all ${analysisMode === 'Personal' ? 'bg-slate-800 border border-slate-700 text-white shadow-lg' : 'text-slate-500'}`}>
                        <Info size={16}/>
                    ...
                    <button onClick={() => setAnalysisMode('External')} className={`py-4 rounded-2xl flex flex-col items-center gap-1 transition-all ${analysisMode === 'External' ? 'bg-indigo-900/40 border border-indigo-500/50 text-indigo-400 shadow-lg' : 'text-slate-500'}`}>
                        <ShieldCheck size={16}/>
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <span className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Motor: {analysis.category === 'External' ? 'Deep Pro' : 'Flash Native'}</span>
                                    </div>
                                </div>
