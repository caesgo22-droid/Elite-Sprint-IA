...
import { Loader2, Zap, Dumbbell, Play, Activity, AlertTriangle, UserCog, X, CheckSquare, Target, Layers, Brain, History, ChevronRight, Share, Clock, Stethoscope, HeartPulse, Info } from 'lucide-react';
...
export const PlanManager: React.FC = () => {
  ...
  // Feedback Modal State for Plan Manager
  ...
  const handleSaveProfile = () => {
    ...
    if (!tempProfile.events.includes(focusEvent)) {
        setFocusEvent(tempProfile.events[0]);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    const plan = await generateTrainingPlan(
      userProfile, 
      { fatigue, sleep, soreness, stress, hydration }, 
      new Date().toLocaleDateString('es-ES'),
      focusEvent,
      acwr || undefined
    );
    if (plan) setPlan(plan);
    else alert("Error al generar plan.");
    setLoading(false);
  };

  const sharePlan = async () => {
     if(!currentPlan) return;
     const text = `PLAN ELITE - ${currentPlan.phase}\nObjetivo: ${currentPlan.weeklyGoal}\nJustificación: ${currentPlan.rationale}\n\n` + 
        currentPlan.sessions.map(s => `${s.day}: ${s.focus} (${s.intensity})\n${s.trackRoutine.join('\n')}`).join('\n\n');
     try { await navigator.clipboard.writeText(text); alert("Plan copiado."); } catch (err) { console.error(err); }
  };

  const toggleEventSelection = (e: string) => {
     const current = tempProfile.events || [];
     if (current.includes(e)) setTempProfile({ ...tempProfile, events: current.filter(ev => ev !== e) });
     else setTempProfile({ ...tempProfile, events: [...current, e] });
  };
  
  const toggleTrainingDay = (day: string) => {
      const current = tempProfile.trainingDays || [];
  ...
  const addInjury = () => {
      const newInjury: Injury = { type: 'Muscular', location: 'Isquios', severity: 'Leve', status: 'Activa' };
      setTempProfile({ ...tempProfile, injuries: [...(tempProfile.injuries || []), newInjury] });
  };
  ...
  const removeInjury = (index: number) => {
      setTempProfile({ ...tempProfile, injuries: tempProfile.injuries?.filter((_, i) => i !== index) });
  };

  const InfoButton = ({ title, text }: { title: string, text: string }) => (
      <button 
        onClick={(e) => { e.stopPropagation(); setActiveTooltip({ title, text }); }} 
        className="text-slate-500 hover:text-cyan-400 ml-1 inline-flex"
      >
          <Info size={12} />
      </button>
  );
      ...
      <div onClick={() => setExpandedDay(isExpanded ? null : session.day)} className={`bg-slate-900/40 border rounded-xl overflow-hidden transition-all duration-300 ${isDone ? 'border-emerald-900/40' : 'border-slate-800'} ${isExpanded ? 'ring-1 ring-cyan-500/50 bg-slate-800/60' : 'hover:bg-slate-800/40'}`}>
        <div className="p-4 flex justify-between items-center cursor-pointer select-none">
          <div className="flex items-center gap-4">
             <div className={`w-12 h-12 rounded-xl flex flex-col items-center justify-center font-bold text-sm bg-slate-800 border border-slate-700 ${isDone ? 'text-emerald-400 border-emerald-900/50' : 'text-slate-200'}`}>
               {isDone ? <CheckSquare size={18} /> : <span className="text-[10px] text-slate-400 uppercase leading-none">{session.day.substring(0, 3)}</span>}
             </div>
             <div>
               <h4 className={`font-bold text-lg tracking-tight ${isDone ? 'text-slate-400 line-through' : 'text-slate-100'}`}>{session.focus}</h4>
               <div className="flex items-center gap-2 mt-1"><span className={`px-2 py-0.5 rounded text-[10px] font-bold border uppercase tracking-wide ${intensityColor}`}>{session.intensity}</span><span className="text-xs text-slate-500">{session.trackRoutine.length} ejercicios</span></div>
             </div>
          </div>
        ...
        {isExpanded && (
          <div className="px-5 pb-5 space-y-5 border-t border-slate-700/50 pt-4 animate-in slide-in-from-top-2">
            ...
            {session.biomechanicsKpi && (<div className="bg-indigo-900/10 border border-indigo-500/20 p-3 rounded-lg flex gap-3 items-start"><Activity size={16} className="text-indigo-400 mt-0.5 shrink-0" /><div><span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider block mb-0.5">KPI Técnico</span><p className="text-sm text-indigo-100">{session.biomechanicsKpi}</p></div></div>)}
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800"><div className="flex items-center gap-2 mb-3 text-xs text-slate-400 uppercase font-bold tracking-wider"><Play size={12} /> Referencias</div><div className="flex flex-wrap gap-2">{session.videoKeywords.map(k => (<a key={k} href={`https://www.youtube.com/results?search_query=sprint+drill+${k.replace(/\s/g, '+')}`} target="_blank" rel="noopener noreferrer" className="text-xs bg-slate-800 hover:bg-slate-700 hover:text-cyan-400 text-slate-300 px-3 py-1.5 rounded-full border border-slate-700 transition-colors">{k}</a>))}</div></div>
            
            <div className="pt-2 border-t border-slate-800">
               <button onClick={(e) => { e.stopPropagation(); setSessionFeedbackModal(session); }} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-2">
                   <CheckSquare size={16}/> {isDone ? 'Ver/Editar Feedback' : 'Registrar Feedback'}
               </button>
            </div>
          </div>
        )}
  ...
  // Internal component for Feedback Modal in PlanManager
  const FeedbackModal = () => {
    ...
    return (
        <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSessionFeedbackModal(null)}>
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-sm space-y-4" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between"><h3 className="font-bold text-white">Feedback {sessionFeedbackModal.day}</h3><button onClick={() => setSessionFeedbackModal(null)}><X/></button></div>
                <div><label className="text-xs font-bold text-slate-400">RPE (1-10)</label><input type="range" min="1" max="10" value={rpe} onChange={e => setRpe(parseInt(e.target.value))} className="w-full accent-cyan-500"/><div className="text-right text-cyan-400 text-xs font-bold">{rpe}</div></div>
                <div><label className="text-xs font-bold text-slate-400">Dolor (0-10)</label><input type="range" min="0" max="10" value={pain} onChange={e => setPain(parseInt(e.target.value))} className="w-full accent-red-500"/><div className="text-right text-red-400 text-xs font-bold">{pain}</div></div>
                <div><label className="text-xs font-bold text-slate-400">Duración (min)</label><input type="number" value={dur} onChange={e => setDur(parseInt(e.target.value))} className="w-full bg-slate-950 p-2 rounded text-white"/></div>
                <div><label className="text-xs font-bold text-slate-400">Superficie</label><select value={srf} onChange={e => setSrf(e.target.value as any)} className="w-full bg-slate-950 p-2 rounded text-white"><option value="Track">Pista</option><option value="Grass">Césped</option><option value="Gym">Gym</option></select></div>
                <div><label className="text-xs font-bold text-slate-400">Notas</label><textarea value={nts} onChange={e => setNts(e.target.value)} className="w-full bg-slate-950 p-2 rounded text-white h-20"/></div>
                <button onClick={save} className="w-full bg-emerald-600 py-3 rounded-lg text-white font-bold">Guardar</button>
            </div>
        </div>
    );
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-16">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 space-y-6">
          ...
          {acwr && (
              <div className="space-y-2">
                  <div className="flex justify-between text-xs text-slate-400 uppercase font-bold tracking-wider items-center">
                      <span>Carga Aguda/Crónica (ACWR) <InfoButton title="ACWR (Acute:Chronic Ratio)" text="Relación entre la fatiga (últimos 7 días) y la condición física (últimos 28 días). >1.5 aumenta riesgo de lesión. <0.8 es desentrenamiento." /></span>
                      <span>{acwr.ratio}</span>
                  </div>
                  <div className="h-4 bg-slate-800 rounded-full overflow-hidden relative">
                      {/* Gauge Bar */}
                      <div 
                        className={`h-full transition-all duration-500 ${acwr.status === 'High Risk' ? 'bg-red-500' : acwr.status === 'Low Load' ? 'bg-yellow-500' : 'bg-emerald-500'}`} 
                        style={{ width: `${Math.min(acwr.ratio * 50, 100)}%` }} // 2.0 = 100%
                      />
                      {/* Zones */}
                      <div className="absolute top-0 bottom-0 left-[40%] w-0.5 bg-slate-900/50 z-10" title="0.8 Min"></div>
                      <div className="absolute top-0 bottom-0 left-[75%] w-0.5 bg-slate-900/50 z-10" title="1.5 Max"></div>
                  </div>
                  <div className={`text-xs text-center font-bold ${acwr.status === 'High Risk' ? 'text-red-400' : acwr.status === 'Low Load' ? 'text-yellow-400' : 'text-emerald-400'}`}>
                      {acwr.status === 'High Risk' ? '⚠️ ZONA DE PELIGRO (>1.5)' : acwr.status === 'Low Load' ? '⚠️ CARGA BAJA (<0.8)' : '✅ ZONA ÓPTIMA (SWEET SPOT)'}
                  </div>
              </div>
          )}
          ...
          <div className="space-y-4 max-w-sm mx-auto pt-4 border-t border-slate-800">
            <div className="space-y-2">
                <div className="flex justify-between text-xs font-medium text-slate-300">
                    <span className="flex items-center">Fatiga <InfoButton title="Fatiga" text="1=Fresco, 10=Exhausto. Nivel de energía general."/></span>
                    <span className="text-cyan-400">{fatigue}/10</span>
                </div>
                <input type="range" min="1" max="10" value={fatigue} onChange={(e) => setFatigue(parseInt(e.target.value))} className="w-full accent-cyan-500 h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
            </div>
            
            <div className="space-y-2">
                <div className="flex justify-between text-xs font-medium text-slate-300">
                    <span className="flex items-center">Sueño <InfoButton title="Calidad de Sueño" text="1=Pésimo, 10=Excelente. Calidad y cantidad de descanso."/></span>
                    <span className="text-indigo-400">{sleep}/10</span>
                </div>
                <input type="range" min="1" max="10" value={sleep} onChange={(e) => setSleep(parseInt(e.target.value))} className="w-full accent-indigo-500 h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
            </div>

            <div className="space-y-2">
                <div className="flex justify-between text-xs font-medium text-slate-300">
                    <span className="flex items-center">Dolor Muscular <InfoButton title="Dolor Muscular (DOMS)" text="1=Sin dolor, 10=Incapacitante. Dolor retardado o agudo."/></span>
                    <span className="text-red-400">{soreness}/10</span>
                </div>
                <input type="range" min="1" max="10" value={soreness} onChange={(e) => setSoreness(parseInt(e.target.value))} className="w-full accent-red-500 h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer" />
            </div>
            
            <div className="pt-4 border-t border-slate-800">
                <label className="text-xs text-slate-400 uppercase font-bold block mb-2">Enfoque de este ciclo</label>
                <div className="flex gap-2">
                    {userProfile.events.map(e => (
                        <button key={e} onClick={() => setFocusEvent(e)} className={`flex-1 py-2 text-xs font-bold rounded border ${focusEvent === e ? 'bg-cyan-900/40 border-cyan-500 text-cyan-400' : 'bg-slate-950 border-slate-800 text-slate-500'}`}>{e}</button>
                    ))}
                </div>
            </div>
          </div>

          <button onClick={handleGenerate} disabled={loading} className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 disabled:opacity-50 text-white font-bold py-4 px-6 rounded-xl flex items-center justify-center gap-3 shadow-lg shadow-cyan-900/20 active:scale-95">
            {loading ? <Loader2 className="animate-spin" /> : <Zap fill="currentColor" />}
            {loading ? 'Consultando Expertos...' : 'Generar Plan Elite'}
          </button>
          
          {/* History List */}
           {planHistoryState.length > 0 && (
              <div className="mt-8 pt-6 border-t border-slate-800">
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2"><History size={12}/> Planes Anteriores</h4>
                  <div className="space-y-2">
                      {planHistoryState.slice(0, 3).map((p: any) => (
                          <div key={p.id} onClick={() => setPlan(p)} className="p-3 bg-slate-900 border border-slate-800 rounded-lg flex justify-between items-center cursor-pointer hover:bg-slate-800">
                              <div><div className="text-sm font-bold text-slate-300">{p.phase}</div><div className="text-xs text-slate-500">{new Date(p.createdAt).toLocaleDateString()}</div></div>
           ...
           <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-2xl border border-slate-700 relative overflow-hidden shadow-2xl">
             <div className="relative z-10">
               ...
               {currentPlan.rationale && (
                   <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-700 mt-4">
                       <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Justificación Técnica (Consejo de Expertos)</h4>
                       <p className="text-xs text-slate-400 italic leading-relaxed">"{currentPlan.rationale}"</p>
                   </div>
               )}

               <button onClick={() => { if(window.confirm("¿Descartar plan actual?")) setPlan(null as any); }} className="mt-4 text-xs text-slate-500 underline decoration-slate-700 hover:text-red-400">Reiniciar Ciclo</button>
             </div>
           </div>
      ...
      {showMacroModal && currentPlan?.rationale && (
          <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setShowMacroModal(false)}>
              <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-sm">
                  <h3 className="text-xl font-bold text-white mb-4">Estrategia Técnica</h3>
                  <p className="text-slate-300 text-sm leading-relaxed">{currentPlan.rationale}</p>
                  <div className="mt-4 pt-4 border-t border-slate-800 text-xs text-slate-500">
                      Decisión basada en ACWR: {currentPlan.acwrStatus?.ratio || 'N/A'} ({currentPlan.acwrStatus?.status || 'N/A'})
                  </div>
              </div>
          </div>
      )}
