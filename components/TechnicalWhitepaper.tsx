    ...
    <div className="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="overflow-y-auto p-6 space-y-8 text-slate-300 text-sm leading-relaxed custom-scrollbar">
          ...
          {/* NEW: OMNI-PERSONA BRAIN */}
          <section className="bg-slate-950/30 p-4 rounded-xl border border-slate-800">
            <h3 className="text-white font-bold flex items-center gap-2 mb-3">
              <Activity size={16} className="text-pink-400" /> Arquitectura "Omni-Persona"
            </h3>
            <p className="text-xs text-slate-400 leading-relaxed mb-3">
              El sistema no es un chatbot estático. Utiliza <strong>Inyección Dinámica de Prompts</strong> para alterar su personalidad y base de conocimientos según el especialista consultado.
            </p>
            <div className="grid grid-cols-3 gap-2 text-[10px] text-center">
              <div className="bg-slate-900 p-2 rounded border border-slate-800">
                <div className="font-bold text-emerald-400">FISIOTERAPEUTA</div>
                <span className="text-slate-500">Enfoque: Anatomía & Protocolos</span>
              </div>
              <div className="bg-slate-900 p-2 rounded border border-slate-800">
                <div className="font-bold text-indigo-400">PSICÓLOGO</div>
                <span className="text-slate-500">Enfoque: CBT & Flow State</span>
              </div>
              <div className="bg-slate-900 p-2 rounded border border-slate-800">
                <div className="font-bold text-cyan-400">BIOMECÁNICO</div>
                <span className="text-slate-500">Enfoque: Vectores & Física</span>
              </div>
            </div>
          </section>
          ...
          <section className="bg-slate-950/30 p-4 rounded-xl border border-slate-800">
            <h3 className="text-white font-bold flex items-center gap-2 mb-3">
              <Eye size={16} className="text-cyan-400" /> Biomecánica V3: Métricas Invisibles
            </h3>
            <ul className="text-xs text-slate-400 space-y-2 list-disc pl-4">
              <li><strong>Real GCT (Tiempos de Contacto):</strong> Medición directa en ms usando interpolación de frames de alta velocidad (60/120/240fps).</li>
              <li><strong>Leg Stiffness (kVert):</strong> Estimación de la rigidez del "resorte humano" basada en la deformación de rodilla durante la fase de amortiguación.</li>
              <li><strong>Asimetría Estructural:</strong> Comparación bilateral (Izq vs Der) de tiempos de vuelo y ángulos de ataque.</li>
            </ul>
          </section>

          {/* NEW: BIOLOGICAL OPTIMIZATION */}
          <section className="bg-slate-950/30 p-4 rounded-xl border border-slate-800">
            <h3 className="text-white font-bold flex items-center gap-2 mb-3">
              <Cpu size={16} className="text-purple-400" /> Optimización Biológica (Bio-Feedback)
            </h3>
            <ul className="text-xs text-slate-400 space-y-2 list-disc pl-4">
              <li><strong>Variabilidad de la Frecuencia Cardíaca (HRV):</strong> Integración con wearables para ajustar la carga de entrenamiento en tiempo real, previniendo el sobreentrenamiento.</li>
              <li><strong>Análisis de Sueño:</strong> Correlación entre la calidad del sueño y la recuperación muscular, impactando la intensidad de las sesiones futuras.</li>
              <li><strong>Niveles de Estrés Percibido (RPE):</strong> Ajuste dinámico de la dificultad de los drills basado en la percepción subjetiva del atleta.</li>
            </ul>
          </section>
