...
const cleanAndParseJSON = (text: string) => {
  ...
  try {
    ...
    console.error("JSON Parse Fail. Original Text:", text);
    return null;
  }
};
...
export const analyzeTechnique = async (images: string[], bioData: any, advancedMetrics: any, analysisMode: string): Promise<any> => {
    ...
    const modelName = isPro ? "gemini-3-pro-image-preview" : "gemini-3-flash-preview";
    ...
    try {
        ...
        // Para modelos de imagen NO usamos responseMimeType según instrucciones
        ...
        Formato: { "phaseDetected": string, "criticalErrors": string[], "correctiveDrills": string[], "coachShouts": string[], "score": number }.`;

        const response = await models.generateContent({
            model: modelName,
            contents: { parts: [...imageParts, { text: prompt }] }
            // NO se incluye responseMimeType para modelos de la serie Nano Banana/Image
        });

        return cleanAndParseJSON(response.text);
    } catch (e: any) {
        console.error("Error en analyzeTechnique:", e.message);
        
        // Manejo de error de clave/entidad según guías
        ...
        if (modelName.includes("pro")) {
            try {
                const fallbackResponse = await models.generateContent({
                    ...
                    contents: { parts: [...images.map(img => ({ inlineData: { mimeType: "image/jpeg", data: img } })), { text: `Analiza técnica de sprint. Responde solo JSON: { "phaseDetected": string, "criticalErrors": string[], "correctiveDrills": string[], "coachShouts": string[], "score": number }.` }] }
                });
...
export const generateNexusInsight = async (logs: any[], readiness: any, lastAnalysis: any, acwr: any): Promise<NexusInsight | null> => {
    ...
    const prompt = `AUDITORÍA MULTIMODAL ELITE. 
            ...
            Como Head Coach, determina si el atleta está en PEAK, RECOVERY o WARNING. Responde solo JSON con status, headline, analysis, recommendation.`;

    try {
        // Los modelos de texto puro sí admiten responseMimeType
        const response = await models.generateContent({
        ...
        if (e.message?.includes("not found") && (window as any).aistudio) {
            (window as any).aistudio.openSelectKey();
        }
        try {
            const fallbackResponse = await models.generateContent({
...
export const chatWithCoach = async (history: any[], message: string, context: any): Promise<any> => {
    ...
    try {
        const response = await models.generateContent({
            ...
            config: {
                systemInstruction: "Eres un Head Coach Nivel V. Responde de forma técnica y directa.",
                tools: [{ googleSearch: {} }] 
            }
        });
